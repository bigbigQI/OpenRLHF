...
class R1RemoteExperienceMaker(RemoteExperienceMaker):
    ...
    def make_experience_list(self, all_prompt_answers: Dict[str, List[str]], **generate_kwargs) -> List[Experience]:
        ...
        args = self.strategy.args
        # generate responses
        samples_list = self.generate_samples(all_prompt_answers, **generate_kwargs)
        torch.distributed.barrier()
    ...
    def generate_samples(self, all_prompt_answers: Dict[str, List[str]], **kwargs) -> List[Samples]:
        ...
        """
        if self.vllm_engines is None:
            raise NotImplementedError("Only VLLM is supported for R1 model")
    ...
    def make_experience(self, samples: R1Samples) -> Experience:
        ...
        if not self.remote_rm_url:
            ...
            if self.custom_reward_func:
                ...
                for rm in self.remote_rm_url:
                    r = remote_rm_fn_ray.remote(rm, queries=queries, prompts=samples.prompts, answers=samples.answers)
                    r_refs.append(r)
